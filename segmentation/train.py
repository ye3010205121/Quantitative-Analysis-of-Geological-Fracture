...
# running
logging.basicConfig(filename='logger.log', level=tf.logging.INFO)
import matplotlib
matplotlib.use('Agg')
...
loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=network, targets=net_output, pos_weight=0.95))

opt = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999,use_locking=True).minimize(loss, var_list=[var for var in tf.trainable_variables()])
